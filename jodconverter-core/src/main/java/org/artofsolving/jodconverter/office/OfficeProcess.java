...
class OfficeProcess {
    ...
    private static final int FIND_PROCESS_RETRY_COUNT = 5;
    ...
    public void deleteProfileDir() {
        if (this.instanceProfileDir != null) {
            try {
                ...
                File oldProfileDir = new File(this.instanceProfileDir.getParentFile(), this.instanceProfileDir.getName() + ".old."
    ...
    public int forciblyTerminate(long retryInterval, long retryTimeout) throws IOException, RetryTimeoutException {
        if (this.pid == PID_UNKNOWN) {
            this.pid = this.processManager.findPid(new ProcessQuery("soffice.bin", this.unoUrl.getAcceptString()));
        }
        this.logger.info(String.format("trying to forcibly terminate process: '" + this.unoUrl + "'"
    ...
    public void start(boolean restart) throws IOException {
        ...
        this.logger.info(String.format("starting process '%s' with acceptString '%s' and profileDir '%s'",
                executable.getAbsolutePath(), this.unoUrl, this.instanceProfileDir));
        this.process = processBuilder.start();

        this.pid = this.processManager.findPid(processQuery);
        int count = 0;
        while (this.pid == PID_NOT_FOUND) {
            this.pid = this.processManager.findPid(processQuery);

            if (count++ >= FIND_PROCESS_RETRY_COUNT) {
                break;
            }
            try {
                Thread.sleep(10);
